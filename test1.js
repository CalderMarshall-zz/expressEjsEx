...
for(i=0; i<players.length; i++){
  ...
  console.log("average", players[i].name, ":", players[i].avgMinutesPlayed);
  if(players[i].starter == false){
	  console.log("bench", players[i].name, ":", players[i].avgMinutesPlayed);
  }
  names.push(players[i].name);
  if (players[i].avgPoints > 10){
	  namesMoreTen.push(players[i].name)
  }
  if (players[i].avgRebounds > 5){
	  namesMoreFive.push(players[i].name)
  }
  namesppm.push(players[i].name + mostPoints);
  loopPoints = players[i].avgMinutesPlayed / players[i].avgPoints
  if (mostPoints < loopPoints ) {
	  mostPoints = loopPoints
	  winner = players[i].name;
  }
  AvgPoints += players[i].avgPoints
}
console.log("Average team points:", AvgPoints / names.length);
console.log("Names of all Players Array", names);
console.log("names of the players who average more than 10 points per game:", namesMoreTen);
console.log("names of the players who average more than 5 rebounds per game:", namesMoreFive);
console.log("player with the most points per minute played:", winner, "points: ", mostPoints)
